...
public class EasterEventHandler implements Listener {
	...
	public EasterEventHandler(Main plugin) {
		...
		if (isEnabled) {
			progress = Languages.langFile.getString("words.progress");
	...
	public void openGUI(Player player) {
		...
		inv.setItem(4, Utils.createItem(Material.NETHER_STAR, 1, Utils.chat("&6"+Languages.langFile.getString("easter.infoItem")), Arrays.asList(Utils.chat("&a"+Languages.langFile.getString("easter.infoItemLore"))), false, true));
		for (int i=0; i < 5; i++) {
	...
	public void onJoin(PlayerJoinEvent e) {
		if (notify)
			plugin.getServer().getScheduler().runTaskLater(plugin, () -> {
				e.getPlayer().sendMessage(Languages.prefix+Utils.chat("&a"+Languages.langFile.getString("easter.eventMessage")));
			}, 20);
	}
	@EventHandler
	public void onClick(InventoryClickEvent e) {
		...
		if (e.getClickedInventory() instanceof MerchantInventory && e.getCurrentItem().hasItemMeta()
			...
			if (eggs[4].hasAchieved(e.getWhoClicked().getUniqueId())) {
				e.getWhoClicked().sendMessage(Utils.chat("&c"+Languages.langFile.getString("easter.restrictAction")));
				e.setCancelled(true);
				return;
			}
	...
	public void onFish(PlayerFishEvent e) {
		if (e.getCaught() != null && e.getCaught() instanceof Item) {
			if (!eggs[1].hasAchieved(e.getPlayer().getUniqueId()) && plugin.random.nextDouble()*100 < 10.0) {
				...
				item = Utils.createItem(item, 1, ChatColor.BLUE+Languages.langFile.getString("easter.blueEgg"), Arrays.asList(ChatColor.YELLOW+Languages.langFile.getString("easter.blueEggLore")), false, true);
				ItemMeta meta = item.getItemMeta();
	...
	public void onItemSpawn(ItemSpawnEvent e) {
		if (e.getEntity().getItemStack().getType() == Material.EGG && e.getEntity().getVelocity().getY() == 0.2 && plugin.random.nextDouble()*100 < 5.0) {
			...
			item.setType(Material.TURTLE_EGG);
			item = Utils.createItem(item, 1, ChatColor.GREEN+Languages.langFile.getString("easter.greenEgg"), Arrays.asList(ChatColor.YELLOW+Languages.langFile.getString("easter.greenEggLore")), false, true);
			ItemMeta meta = item.getItemMeta();
		...
		if (e.getEntity() instanceof Player && droppedEggs.containsKey(e.getItem().getUniqueId())) {
			if (eggs[0].hasAchieved(e.getEntity().getUniqueId())) {
				if (!droppedEggs.get(e.getItem().getUniqueId()).contains(e.getEntity().getUniqueId())) {
					e.getEntity().sendMessage(Utils.chat("&c"+Languages.langFile.getString("easter.restrictAction")));
	...
	public void onCreatureSpawn(CreatureSpawnEvent e) {
		if (e.isCancelled() || e.getEntityType() != EntityType.CHICKEN || !(e.getSpawnReason() == SpawnReason.NATURAL || e.getSpawnReason() == SpawnReason.BREEDING || e.getSpawnReason() == SpawnReason.EGG) || GlobalSpawner.noSpawnWorlds.contains(e.getLocation().getWorld())
		...
		plugin.handler.addEntity(new KillerChicken(zombie, plugin));
		e.getEntity().remove();
	}
	public class EggGoal {
		...
		public EggGoal(int index) {
			...
			switch (index) {
			case 0:
				name = Utils.chat(ChatColor.GREEN+Languages.langFile.getString("easter.greenEgg"));
				description = Utils.chat(ChatColor.YELLOW+Languages.langFile.getString("easter.greenEggGoal"));
				cmd = 100023;
				break;
			case 1:
				name = Utils.chat(ChatColor.BLUE+Languages.langFile.getString("easter.blueEgg"));
				description = Utils.chat(ChatColor.YELLOW+Languages.langFile.getString("easter.blueEggGoal"));
				cmd = 100024;
				break;
			case 2:
				name = Utils.chat(ChatColor.RED+Languages.langFile.getString("easter.redEgg"));
				description = Utils.chat(ChatColor.YELLOW+Languages.langFile.getString("easter.redEggGoal"));
				cmd = 100025;
				break;
			case 3:
				name = Utils.chat(ChatColor.GOLD+Languages.langFile.getString("easter.orangeEgg"));
				description = Utils.chat(ChatColor.YELLOW+Languages.langFile.getString("easter.orangeEggGoal"));
				cmd = 100026;
				break;
			case 4:
				name = Utils.chat(ChatColor.LIGHT_PURPLE+Languages.langFile.getString("easter.purpleEgg"));
				description = Utils.chat(ChatColor.YELLOW+Languages.langFile.getString("easter.purpleEggGoal"));
				cmd = 100027;
				break;
			}
		...
		public void addProgress(UUID uuid, int amount) {
			...
			if (hasAchieved(uuid) && Bukkit.getPlayer(uuid) != null)
				Bukkit.getPlayer(uuid).sendMessage(Languages.prefix+Utils.chat("&a"+Languages.langFile.getString("easter.discoverEgg")+" "+name+"!"));
		}
