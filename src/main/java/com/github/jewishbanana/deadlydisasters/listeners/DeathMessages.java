...
public class DeathMessages implements Listener {
	...
	public static Set<Monsoon> monsoons = new HashSet<>();
	public static Set<InfestedCaves> infestedcaves = new HashSet<>();
	public static Set<Supernova> supernovas = new HashSet<>();
	public static Set<LandSlide> landslides = new HashSet<>();
	...
	public void onDeath(PlayerDeathEvent e) {
		...
		if (cause != DamageCause.VOID && cause != DamageCause.SUICIDE) {
			...
			for (InfestedCaves cave : infestedcaves)
				if (p.getWorld().equals(cave.getLocation().getWorld()) && p.getLocation().distance(cave.getLocation()) <= cave.getSize()) {
		...
		if (p.hasMetadata("dd-plague")) {
			...
			if (p.hasMetadata("dd-plaguedeath")) {
				e.setDeathMessage(e.getEntity().getName()+" "+Languages.getString("deaths.plague"));
		...
		if (p.hasMetadata("dd-lostsouldeath")) {
			e.setDeathMessage(p.getName()+" "+Languages.getString("deaths.soulDeath"));
			...
			villager.setCustomName(p.getName()+"'"+Languages.getString("deaths.corpse"));
			return;
		}
		if (p.hasMetadata("dd-unstablerift")) {
			p.removeMetadata("dd-unstablerift", plugin);
			e.setDeathMessage(p.getName()+" "+Languages.getString("deaths.rift"));
			return;
		}
		if (p.hasMetadata("dd-supernova")) {
			p.removeMetadata("dd-supernova", plugin);
			e.setDeathMessage(p.getName()+" "+Languages.getString("deaths.supernova"));
		...
		if (p.hasMetadata("dd-meteorcrush")) {
			...
			e.setDeathMessage(p.getName()+" "+Languages.getString("deaths.meteor"));
			Metrics.incrementValue(Metrics.disasterKillMap, Disaster.METEORSHOWERS.getMetricsLabel());
			return;
		}
		if (p.hasMetadata("dd-monsoondrown")) {
			p.removeMetadata("dd-monsoondrown", plugin);
			e.setDeathMessage(p.getName()+" "+Languages.getString("deaths.monsoon"));
		...
		if (p.hasMetadata("dd-caveincrush")) {
			...
			e.setDeathMessage(p.getName()+" "+Languages.getString("deaths.cavein"));
		...
		if (p.hasMetadata("dd-endwormfangs")) {
			...
			e.setDeathMessage(p.getName()+" "+Languages.getString("deaths.worm"));
			return;
		}
		if (p.hasMetadata("dd-purgedeath")) {
			p.removeMetadata("dd-purgedeath", plugin);
			e.setDeathMessage(p.getName()+" "+Languages.getString("deaths.purge"));
		...
		if (p.hasMetadata("dd-leechdeath")) {
			...
			e.setDeathMessage(p.getName()+" "+Languages.getString("deaths.genericMob")+' '+Languages.getString("entities.shadowLeech"));
			return;
		}
		if (p.hasMetadata("dd-bloodleechdeath")) {
			p.removeMetadata("dd-bloodleechdeath", plugin);
			e.setDeathMessage(p.getName()+" "+Languages.getString("deaths.genericMob")+' '+Languages.getString("entities.bloodyLeech"));
			return;
		}
		if (p.hasMetadata("dd-devourdeath")) {
			p.removeMetadata("dd-devourdeath", plugin);
			e.setDeathMessage(p.getName()+" "+Languages.getString("deaths.genericMob")+' '+Languages.getString("entities.infestedDevourer"));
			return;
		}
		if (p.hasMetadata("dd-infestedwormdeath")) {
			p.removeMetadata("dd-infestedwormdeath", plugin);
			e.setDeathMessage(p.getName()+" "+Languages.getString("deaths.infestedWormDeath"));
			return;
		}
		if (p.hasMetadata("dd-sandstormdeath")) {
			p.removeMetadata("dd-sandstormdeath", plugin);
			e.setDeathMessage(p.getName()+" "+Languages.getString("deaths.sandstorm"));
		...
		if (p.hasMetadata("dd-acidstormdeath")) {
			...
			e.setDeathMessage(p.getName()+" "+Languages.getString("deaths.acidstorm"));
		...
		if (p.hasMetadata("dd-blizzarddeath")) {
			...
			e.setDeathMessage(p.getName()+" "+Languages.getString("deaths.blizzard"));
		...
		if (p.hasMetadata("dd-geyserdeath")) {
			...
			e.setDeathMessage(p.getName()+" "+Languages.getString("deaths.waterGeyser"));
		...
		if (p.hasMetadata("dd-tsunamideath")) {
			...
			e.setDeathMessage(p.getName()+" "+Languages.getString("deaths.tsunami"));
			Metrics.incrementValue(Metrics.disasterKillMap, Disaster.TSUNAMI.getMetricsLabel());
			return;
		}
		if (p.hasMetadata("dd-bloodsacrifice")) {
			p.removeMetadata("dd-bloodsacrifice", plugin);
			e.setDeathMessage(p.getName()+" "+Languages.getString("deaths.bloodSacrifice"));
			return;
		}
		if (p.hasMetadata("dd-bloodsacrificesuicide")) {
			p.removeMetadata("dd-bloodsacrificesuicide", plugin);
			e.setDeathMessage(p.getName()+" "+Languages.getString("deaths.bloodSacrificeSuicide"));
			return;
		}
		...
		if (p.hasMetadata("dd-avalanche")) {
			p.removeMetadata("dd-avalanche", plugin);
			e.setDeathMessage(p.getName()+" "+Languages.getString("deaths.avalancheDeath"));
			return;
		}
		if (p.hasMetadata("dd-landslide")) {
			p.removeMetadata("dd-landslide", plugin);
			e.setDeathMessage(p.getName()+" "+Languages.getString("deaths.landslideDeath"));
			return;
		}
		...
		if (cause == DamageCause.FALL || cause == DamageCause.LAVA) {
			for (Sinkhole obj : sinkholes) {
				...
				if (obj.getLocation().getWorld().equals(world) && p.getLocation().getBlockX() >= x-radius && p.getLocation().getBlockX() <= x+radius && p.getLocation().getBlockZ() >= z-radius && p.getLocation().getBlockZ() <= z+radius && p.getLocation().getBlockY() <= y) {
					e.setDeathMessage(p.getName()+" "+Languages.getString("deaths.sinkhole"));
			...
			for (Earthquake obj : earthquakes) {
				...
				if (obj.getLocation().getWorld().equals(world) && loc.getBlockX() >= x-radius && loc.getBlockX() <= x+radius && loc.getBlockZ() >= z-radius && loc.getBlockZ() <= z+radius && loc.getBlockY() < y) {
					e.setDeathMessage(p.getName()+" "+Languages.getString("deaths.earthquake"));
		...
		if (cause == DamageCause.FALL) {
			for (Tornado obj : tornados)
				if (obj.isEntityInvolved(p.getUniqueId())) {
					e.setDeathMessage(p.getName()+" "+Languages.getString("deaths.tornado"));
			...
			for (ExtremeWinds obj : extremewinds)
				if (obj.getWorld().equals(world) && obj.isEntityInvolved(p.getUniqueId())) {
					e.setDeathMessage(p.getName()+" "+Languages.getString("deaths.extremewinds"));
		...
		if (cause == DamageCause.LAVA && world.getEnvironment() == Environment.NETHER) {
			for (Geyser obj : geysers) {
				...
				if (obj.getLocation().getWorld().equals(world) && p.getLocation().getBlockX() >= x-2 && p.getLocation().getBlockX() <= x+1 && p.getLocation().getBlockZ() >= z-2 && p.getLocation().getBlockZ() <= z+1 && p.getLocation().getBlockY() >= mem
					...
					e.setDeathMessage(p.getName()+" "+Languages.getString("deaths.lavaGeyser"));
					Metrics.incrementValue(Metrics.disasterKillMap, Disaster.GEYSER.getMetricsLabel());
					return;
				}
			}
		...
		if (cause == DamageCause.SUFFOCATION) {
			for (InfestedCaves cave : infestedcaves)
				if (p.getWorld().equals(cave.getLocation().getWorld()) && p.getLocation().distance(cave.getLocation()) <= cave.getSize() && p.getLocation().add(0,1,0).getBlock().getType() == cave.getMaterial()) {
					e.setDeathMessage(p.getName()+" "+Languages.getString("deaths.infestedCave"));
					return;
				}
		}
		if (cause == DamageCause.FALL || cause == DamageCause.LIGHTNING) {
			for (Hurricane obj : hurricanes)
				if (p.getLocation().distance(obj.getLocation()) <= obj.getSize()) {
					e.setDeathMessage(p.getName()+" "+Languages.getString("deaths.hurricane"));
