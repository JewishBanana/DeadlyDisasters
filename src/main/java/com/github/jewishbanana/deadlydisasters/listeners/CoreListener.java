...
public class CoreListener implements Listener {
			...
			public void run() {
				...
				if (e.getPlayer().hasPermission("deadlydisasters.updatenotify") && dataFile.getBoolean("data.firstStart")) {
					...
					if (plugin.mcVersion < 1.16)
						e.getPlayer().sendMessage(Languages.prefix+Utils.chat("&c"+Languages.getString("internal.olderVersion")));
				}
				...
				if (warnForKick.contains(uuid)) {
					e.getPlayer().sendMessage(Utils.chat(Languages.prefix+"&c"+Languages.getString("internal.allowFlight")));
					warnForKick.remove(uuid);
				}
				if (catalogNotifyBool && !catalogNotify.contains(uuid)) {
					e.getPlayer().sendMessage(Languages.prefix+ChatColor.GREEN+Languages.getString("internal.catalogUpdate")+Utils.chat(" &3(/disasters catalog)"));
					catalogNotify.add(uuid);
				}
				if (e.getPlayer().isOp() && !joinedServer.contains(uuid)) {
					if (!favoredDisaster)
						e.getPlayer().sendMessage(Languages.prefix+ChatColor.AQUA+Languages.getString("internal.favorDisaster")+Utils.chat(" &3(/disasters favor <disaster>)"));
					if (!dislikedDisaster)
						e.getPlayer().sendMessage(Languages.prefix+ChatColor.AQUA+Languages.getString("internal.dislikeDisaster")+Utils.chat(" &3(/disasters dislike <disaster>)"));
	...
	public void onBlockBreak(BlockBreakEvent event) {
		if (InfestedWorm.immuneBlocks.contains(event.getBlock()) || Santa.snowGlobeBlocks.contains(event.getBlock()) || EasterBunny.easterBasketBlocks.contains(event.getBlock()) || PumpkinKing.pumpkinBasketBlocks.contains(event.getBlock())) {
		...
		if (fb.hasMetadata("dd-fb")) {
			...
			if (fallingBlocks.containsKey(fb.getUniqueId())) {
				...
				if (disaster instanceof SolarStorm) {
					disaster.addBlockToList(event.getBlock(), event.getBlock().getState());
					...
					if (mat == Material.DIRT || mat == Material.GRASS_BLOCK || mat == Material.PODZOL) {
						...
						disaster.addBlockWithTopToList(target, target.getState());
	...
	public void bowShoot(EntityShootBowEvent e) {
		if (e.getBow() == null || e.getForce() < 0.8 || !e.getBow().hasItemMeta())
			return;
	...
	public void onConsume(PlayerItemConsumeEvent e) {
		...
		if (e.getItem().getItemMeta().getPersistentDataContainer().has(ItemsHandler.plagueCureKey, PersistentDataType.BYTE)) {
			BlackPlague.cureEntity(e.getPlayer());
			e.getPlayer().sendMessage(ChatColor.GREEN+Languages.getString("misc.cureMessage"));
			return;
		}
	}
	@EventHandler
	public void onPotionSplash(PotionSplashEvent e) {
		if (e.getPotion().getItem().hasItemMeta() && e.getPotion().getItem().getItemMeta().getPersistentDataContainer().has(ItemsHandler.plagueCureKey, PersistentDataType.BYTE))
	...
	public void onExplode(EntityExplodeEvent e) {
		...
		for (Block b : e.blockList()) {
			if (Utils.isZoneProtected(b.getLocation()) || Utils.passStrengthTest(b.getType()))
				continue;
			disaster.addBlockWithTopToList(b, b.getState());
			if (plugin.CProtect)
