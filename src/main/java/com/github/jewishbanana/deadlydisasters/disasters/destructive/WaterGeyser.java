...
public class WaterGeyser extends Disaster {
	...
	public Location findPossiblePosition(Location initial) {
		if (initial == null)
			return null;
		Location temp = new Location(initial.getWorld(), initial.getX(), random.nextInt(-60, -55), initial.getZ());
		int count = 0;
	...
	public void start() {
		...
		scheduleTask(new BukkitRunnable() {
			...
			public void run() {
				if (interval-- == 0) {
					createGeyser(BlockUtils.getCenterOfBlock(location.clone().add(Utils.getRandomizedVector(1, 0, 1).multiply(spawnRange)).getBlock()), disasterRange, random.nextInt(250, 320), 3.0 * speed);
