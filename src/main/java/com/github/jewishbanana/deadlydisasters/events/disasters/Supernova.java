		...
		new RepeatingTask(plugin, 0, 1) {
			...
			public void run() {
				if (tick > size) {
					...
					awardPlayersInSet(survivingPlayers, 1, "disasters.survival.supernova");
					if (level >= 6)
						awardPlayersInSet(survivingPlayers, 1, "disasters.survival.level_six", 4);
					...
					for (int y = -tick; y < tick; y++)
						for (int z = -tick; z < tick; z++) {
							...
							if (tick > size-2 && rand.nextInt(8) == 0) {
								...
								addBlockWithTopToList(b, b.getState());
								b.setType(mat);
								blocksDestroyed++;
								continue;
							}
