...
public class AcidStorm extends WeatherDisaster {
	...
	public AcidStorm(int level) {
		...
		delay = configFile.getInt("acidstorm.start_delay") * 20;
		damage = configFile.getDouble("acidstorm.damage");
		particleRange = configFile.getInt("acidstorm.particle_max_distance");
		particleYRange = configFile.getInt("acidstorm.particle_Y_range");
		particleMultiplier = 0.25 * configFile.getDouble("acidstorm.particle_multiplier");
		blockDamageRange = configFile.getInt("acidstorm.block_damage_range");
		blockChangeRate = 0.03 * configFile.getDouble("acidstorm.block_change_rate");
		slimeRate = 0.075 * configFile.getDouble("acidstorm.slime_spawn_rate");
		poisonCrops = configFile.getBoolean("acidstorm.poison_crops");
		volume = configFile.getDouble("acidstorm.volume");
		for (String effect : configFile.getConfigurationSection("acidstorm.effects").getKeys(false))
			if (PotionEffectType.getByName(effect) != null)
				potionEffects.putIfAbsent(PotionEffectType.getByName(effect), configFile.getInt("acidstorm.effects."+effect));
		for (String material : configFile.getConfigurationSection("acidstorm.block_changes").getKeys(false))
			if (Material.getMaterial(material.toUpperCase()) != null) {
				if (Material.getMaterial(configFile.getString("acidstorm.block_changes."+material).toUpperCase()) != null)
					blockChanges.putIfAbsent(Material.getMaterial(material.toUpperCase()), Material.getMaterial(configFile.getString("acidstorm.block_changes."+material).toUpperCase()));
