						...
						public void run() {
							...
							if (placements.isEmpty()) {
								...
								plugin.getServer().getScheduler().runTaskLater(plugin, () -> {
									awardPlayersInSet(survivingPlayers, 1, "disasters.survival.sinkhole");
									if (level >= 6)
										awardPlayersInSet(survivingPlayers, 1, "disasters.survival.level_six", 0);
		...
		for (int i=rand.nextInt(3)+1; i > 0; i--) {
			if (!Utils.passStrengthTest(b.getType()) && !Utils.isZoneProtected(b.getLocation())) {
				...
				if (!Utils.passStrengthTest(b2.getType()) && !Utils.isZoneProtected(b2.getLocation())) {
					...
					classInstance.addBlockToList(b2, b2.getState());
