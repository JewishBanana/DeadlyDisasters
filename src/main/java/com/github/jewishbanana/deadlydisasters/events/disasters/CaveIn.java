...
public class CaveIn extends DestructionDisaster {
	...
	public void start(Location loc, Player p) {
		...
		addPlayersToSurvivalChannel(loc.clone().subtract(0,radius,0), radius, survivingPlayers);
				...
				if (placements.isEmpty()) {
					plugin.getServer().getScheduler().runTaskLater(plugin, new Runnable() {
						...
						public void run() {
							...
							awardPlayersInSet(survivingPlayers, 1, "disasters.survival.cavein");
							if (level >= 6)
								awardPlayersInSet(survivingPlayers, 1, "disasters.survival.level_six", 3);
		...
		new RepeatingTask(plugin, 0, 2) {
			@Override
			public void run() {
				if (placements.isEmpty() && fallingRoof.isEmpty()) {
					cancel();
					return;
				}
		...
		if (depth > 0) {
			...
			if (!Utils.passStrengthTest(b.getType()) && !Utils.isZoneProtected(loc)) {
				...
				if (state instanceof InventoryHolder)
					CoreListener.addBlockInventory(fb, ((InventoryHolder) state).getInventory().getContents());
