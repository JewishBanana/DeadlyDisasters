...
public class EndWorm extends CustomEntity {
		...
		if (anim && cooldown == 0) {
			...
			if (damageEntity != null && stands[4] == null) {
				if (damageEntity.isDead()) {
					...
					if (damageEntity.getKiller() != null) {
						CustomDropsFactory.generateDrops(damageEntity.getLocation(), entityType);
						plugin.achievementsHandler.awardProgress(damageEntity.getKiller().getUniqueId(), "mobs.slayer.void_mobs", 1);
					}
					clean();
					entity.remove();
				}
	...
	public void function(Iterator<CustomEntity> it) {
		...
		if (entity.getLocation().getBlockY() < -64 && lastTarget != null && lastTarget instanceof Player) {
			plugin.achievementsHandler.awardProgress(lastTarget.getUniqueId(), "master.series.void_master", 1, 4);
			entity.remove();
			return;
		}
		if (cooldown > 0) {
