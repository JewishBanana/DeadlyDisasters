...
public class ElfPet extends CustomEntity {
	...
	public void function(Iterator<CustomEntity> it) {
		...
		if (rand.nextInt(10) == 0)
			if (rand.nextInt(4) == 0)
				entity.getWorld().playSound(entity.getLocation(), Sound.ENTITY_VILLAGER_YES, SoundCategory.HOSTILE, 0.25f, 2);
			else
				entity.getWorld().playSound(entity.getLocation(), Sound.ENTITY_VILLAGER_AMBIENT, SoundCategory.HOSTILE, 0.25f, 2);
	}
	@Override
	public void clean() {
		entity.remove();
	...
	public boolean spawnCondition(Location loc) {
		if (plugin.seasonsHandler.isActive ? SeasonsHandler.getSeasonsAPI().getSeason(loc.getWorld()) == me.casperge.realisticseasons.season.Season.WINTER : loc.getBlock().getTemperature() <= 0.15)
			return true;
		return false;
	}
	public Entity spawnEntity(Location loc) {
		Entity entity = (Mob) loc.getWorld().spawnEntity(loc, EntityType.ZOMBIE);
		handler.addEntity(new Elf((Zombie) entity, Main.getInstance(), Main.getInstance().random));
		return entity;
	}
}
