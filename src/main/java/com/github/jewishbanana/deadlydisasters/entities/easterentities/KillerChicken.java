...
public class KillerChicken extends CustomEntity {
	...
	public KillerChicken(Zombie entity, Main plugin) {
		...
		if (entity.getCustomName() == null)
			entity.setCustomName(Languages.getString("easter.killerChicken"));
		
		chicken = (Chicken) entity.getWorld().spawnEntity(entity.getLocation(), EntityType.CHICKEN);
		chicken.getAttribute(Attribute.GENERIC_MAX_HEALTH).setBaseValue(entityType.getHealth());
		chicken.setHealth(entityType.getHealth());
	...
	public Entity spawnEntity(Location loc) {
		...
		CustomEntity.handler.addEntity(new KillerChicken(entity, Main.getInstance()));
		return entity;
	}
}
