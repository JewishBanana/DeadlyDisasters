...
public class InfestedTribesman extends CustomEntity {
	
	private boolean archer,noStack;
	private int cooldown = 2, maxStack = 0;
	private Random rand;

	public InfestedTribesman() {
	}
	@SuppressWarnings("deprecation")
	public InfestedTribesman(Zombie entity, Main plugin, Random rand) {
		super(entity, plugin);
		this.rand = rand;
		...
		ItemStack head = null;
		if (rand.nextInt(3) != 0)
			head = CustomHead.INFESTEDTRIBESMAN1.getHead();
		else
			head = CustomHead.INFESTEDTRIBESMAN2.getHead();
		...
		int green = rand.nextInt(60)+25;
		for (int i=0; i < 3; i++) {
			LeatherArmorMeta meta = (LeatherArmorMeta) armor[i].getItemMeta();
			meta.setColor(Color.fromBGR(green-20, green, 1));
		...
		entity.setMetadata("dd-infestedtribesman", plugin.fixedData);
		if (entity.getCustomName() == null)
			entity.setCustomName(Languages.getString("entities.infestedTribesman"));
		
		if (rand.nextDouble()*100 < (double) entityType.grabCustomSetting("archer_spawn_chance")) {
			entity.getEquipment().setItemInMainHand(new ItemStack(Material.BOW));
			archer = true;
			entity.setMetadata("dd-infestedtribesmanarcher", plugin.fixedData);
			if (rand.nextInt(4) == 0)
				noStack = true;
	...
	public void tick() {
		...
		if (entity.getNoDamageTicks() == 20 && plugin.mcVersion >= 1.19)
			entity.getWorld().playSound(entity.getLocation(), Sound.BLOCK_SCULK_SENSOR_BREAK, SoundCategory.HOSTILE, 1, 1.5F);
	...
	public void function(Iterator<CustomEntity> it) {
		...
		if (archer) {
			if (!noStack && !entity.isInsideVehicle())
				for (Entity e : entity.getNearbyEntities(1, 1, 1))
					if (!e.isInsideVehicle() && e.getPassengers().size() <= maxStack && e.hasMetadata("dd-infestedtribesman") && !e.hasMetadata("dd-infestedtribesmanarcher"))
		...
		if (rand.nextInt(6) == 0 && plugin.mcVersion >= 1.19)
			entity.getWorld().playSound(entity.getLocation(), Sound.BLOCK_SCULK_CATALYST_BLOOM, SoundCategory.HOSTILE, 1, rand.nextFloat()+0.5F);
	}
	@Override
	public void clean() {
	}
