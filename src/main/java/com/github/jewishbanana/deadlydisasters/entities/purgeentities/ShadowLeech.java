...
public class ShadowLeech extends CustomEntity {
	...
	public ShadowLeech(Zombie entity, Main plugin, Random rand) {
		...
		if (rand.nextDouble()*100 < (double) entityType.grabCustomSetting("blood_leech_conversion")) {
			bloodleech = true;
			damage = entityType.getDamage()*1.5;
	...
	public void tick() {
		...
		if (attached) {
			...
			if (loc.getWorld().equals(entity.getTarget().getWorld()) && loc.distanceSquared(entity.getLocation()) < 0.3) {
				...
				team = sb.registerNewTeam(("col-"+entity.getUniqueId()).substring(0, 16));
	...
	private void initZombie() {
		...
		if (!bloodleech) {
			...
			entity.setCustomName(Languages.getString("entities.shadowLeech"));
		} else {
			entity.getAttribute(Attribute.GENERIC_MAX_HEALTH).setBaseValue(entityType.getHealth()*2);
			entity.setHealth(entityType.getHealth()*2);
			entity.getAttribute(Attribute.GENERIC_MOVEMENT_SPEED).setBaseValue(.3);
			entity.setCustomName(Languages.getString("entities.bloodyLeech"));
		}
	}
