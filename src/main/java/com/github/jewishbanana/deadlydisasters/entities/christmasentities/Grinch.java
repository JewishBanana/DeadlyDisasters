...
public class Grinch extends CustomEntity {
	...
	public void function(Iterator<CustomEntity> it) {
		...
		if (entity.isDead()) {
			...
			if (entity.getKiller() != null) {
				...
				if (rand.nextDouble()*100 < (double) entityType.grabCustomSetting("candy_cane_droprate"))
	...
	public Entity spawnEntity(Location loc) {
		...
		handler.addEntity(new Grinch(entity, Main.getInstance(), Main.getInstance().random));
		return entity;
	}
}
