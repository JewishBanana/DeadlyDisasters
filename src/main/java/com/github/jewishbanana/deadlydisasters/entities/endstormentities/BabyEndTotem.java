...
public class BabyEndTotem extends CustomEntity {
	...
	public BabyEndTotem(Mob entity, Main plugin, Random rand) {
		...
		if (Main.getInstance().dataFile.contains("customentities."+entity.getUniqueId()+".name")) {
			entity.setCustomName(Main.getInstance().dataFile.getString("customentities."+entity.getUniqueId()+".name"));
			stands[3].setCustomName(Main.getInstance().dataFile.getString("customentities."+entity.getUniqueId()+".name"));
		} else {
			entity.setCustomName(Languages.getString("entities.endTotem"));
			stands[3].setCustomName(Languages.getString("entities.endTotem"));
		}
	...
	private void spawnStands(int index) {
	...
	public Entity spawnEntity(Location loc) {
		...
		handler.addEntity(new BabyEndTotem(entity, Main.getInstance(), Main.getInstance().random));
		return entity;
	}
}
