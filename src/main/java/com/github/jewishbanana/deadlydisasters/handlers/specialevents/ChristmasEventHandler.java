...
public class ChristmasEventHandler extends SpecialEvent implements Listener {
	...
	public void openGUI(Player player) {
		Inventory inv = Bukkit.createInventory(null, 27, Utils.convertString("&9DeadlyDisasters &cC&ah&cr&ai&cs&at&cm&aa&cs &aE&cv&ae&cn&at"));
		ItemStack glass = Utils.createItem(Material.GRAY_STAINED_GLASS_PANE, 1, " ", null, false, true);
		for (int i=0; i < 27; i++)
			inv.setItem(i, glass);
		inv.setItem(4, Utils.createItem(Material.NETHER_STAR, 1, Utils.convertString("&6"+Languages.getString("christmas.infoItem")), Arrays.asList(Utils.convertString("&a"+Languages.getString("christmas.infoItemLore"))), false, true));
	...
	public void onJoin(PlayerJoinEvent e) {
		if (notify)
			plugin.getServer().getScheduler().runTaskLater(plugin, () -> {
				e.getPlayer().sendMessage(Languages.prefix+Utils.convertString("&a"+Languages.getString("christmas.eventMessage")));
			}, 20);
	}
	@EventHandler
	...
	public void onCreatureSpawn(CreatureSpawnEvent e) {
		if (e.isCancelled() || e.getSpawnReason() != SpawnReason.NATURAL || !Utils.isEnvironment(e.getLocation().getWorld(), Environment.NORMAL) || GlobalSpawner.noSpawnWorlds.contains(e.getLocation().getWorld()) || !(e.getEntity() instanceof Monster) || !e.getLocation().getBlock().getRelative(BlockFace.DOWN).getType().isSolid())
			return;
		...
		for (int i : order)
			switch (i) {
			case 1:
				if (rand.nextDouble() * 100 < CustomEntityType.GHOUL.getSpawnRate()) {
				...
				break;
			case 2:
				if (rand.nextDouble() * 100 < CustomEntityType.SCARECROW.getSpawnRate()) {
				...
				break;
			case 3:
				if (rand.nextDouble() * 100 < CustomEntityType.VAMPIRE.getSpawnRate()) {
				...
				break;
			case 4:
				if (rand.nextDouble() * 100 < CustomEntityType.PSYCO.getSpawnRate()) {
