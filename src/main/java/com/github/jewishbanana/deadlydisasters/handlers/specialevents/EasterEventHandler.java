...
public class EasterEventHandler extends SpecialEvent implements Listener {
	...
	public EasterEventHandler(Main plugin) {
		...
		case 2023:
			...
			break;
		case 2024:
			if ((date.getMonth() == Month.MARCH && date.getDayOfMonth() >= 24) || (date.getMonth() == Month.APRIL && date.getDayOfMonth() <= 7))
				isEnabled = true;
			break;
		case 2025:
			if (date.getMonth() == Month.APRIL && date.getDayOfMonth() <= 27)
				isEnabled = true;
			break;
		case 2026:
			if ((date.getMonth() == Month.MARCH && date.getDayOfMonth() >= 16) || (date.getMonth() == Month.APRIL && date.getDayOfMonth() <= 12))
				isEnabled = true;
			break;
		case 2027:
			if ((date.getMonth() == Month.MARCH && date.getDayOfMonth() >= 7) || (date.getMonth() == Month.APRIL && date.getDayOfMonth() <= 4))
				isEnabled = true;
			break;
		case 2028:
			if ((date.getMonth() == Month.MARCH && date.getDayOfMonth() >= 26) || (date.getMonth() == Month.APRIL && date.getDayOfMonth() <= 23))
				isEnabled = true;
			break;
		case 2029:
			if ((date.getMonth() == Month.MARCH && date.getDayOfMonth() >= 11) || (date.getMonth() == Month.APRIL && date.getDayOfMonth() <= 8))
				isEnabled = true;
			break;
		case 2030:
			if (date.getMonth() == Month.APRIL && date.getDayOfMonth() <= 28)
				isEnabled = true;
			break;
		default:
			if ((date.getMonth() == Month.MARCH && date.getDayOfMonth() >= 20) || (date.getMonth() == Month.APRIL && date.getDayOfMonth() <= 17))
				isEnabled = true;
			break;
		}
		if (isEnabled) {
	...
	public void onFish(PlayerFishEvent e) {
		if (e.getCaught() != null && e.getCaught() instanceof Item && !e.isCancelled()) {
	...
	public void onCreatureSpawn(CreatureSpawnEvent e) {
		if (e.isCancelled())
			return;
		if (e.getLocation().getBlockY() > 150 && plugin.random.nextDouble()*100 < CustomEntityType.RAMPAGINGGOAT.getSpawnRate()) {
		...
		if (e.getEntityType() != EntityType.CHICKEN || !(e.getSpawnReason() == SpawnReason.NATURAL || e.getSpawnReason() == SpawnReason.BREEDING || e.getSpawnReason() == SpawnReason.EGG) || GlobalSpawner.noSpawnWorlds.contains(e.getLocation().getWorld())
				|| plugin.random.nextDouble()*100 >= CustomEntityType.KILLERCHICKEN.getSpawnRate())
			return;
