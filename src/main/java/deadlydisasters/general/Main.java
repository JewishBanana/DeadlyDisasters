...
import deadlydisasters.listeners.LootGenerateListener;
import deadlydisasters.listeners.TownyListener;
import deadlydisasters.listeners.spawners.GlobalSpawner;
import deadlydisasters.listeners.spawners.InfestedSpawner;
...
public class Main extends JavaPlugin {
	...
	public boolean updateNotify,firstSetup,firstAfterUpdate,upgradeToPro,debug;
	...
	/* [Errors]:
	 * 00: Unable to save dataFile (default save)
	 * 01: Could not connect to spigot
	 * 02: Could not read update from spigot
	 * 03: Unable to submit initial database request
	 * 04: Unable to do routine database request
	 * 05: Unable to read latest.log for errors
	 * 06: Unable to submit errorLog
	 * 07: Could not update config properly so reset values
	 * 
	 * [Updates]
	 * - Small patch to tc updateTimerList
	 */
	...
	public void onEnable() {
		...
		if (mcVersion >= 1.19)
			new InfestedSpawner(this, handler);
		...
		getServer().getScheduler().runTaskAsynchronously(this, new Runnable() {
			public void run() {
				try {
					...
					if (new File(plugin.getDataFolder().getAbsolutePath(), "blocks.yml").exists())
						ConfigUpdater.update(plugin, getResource("files/blocks.yml"), new File(getDataFolder().getAbsolutePath(), "blocks.yml"), Arrays.asList(""));
