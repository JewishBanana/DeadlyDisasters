...
public class Tornado extends DestructionDisaster {
	...
	private UUID addBlock(Block b, FixedMetadataValue fixdata, double force, Random rand) {
		...
		Material prevMaterial = b.getType();
		...
		if (b.getState() instanceof InventoryHolder)
			...
			addBlockToList(b, new Material[] {prevMaterial, Material.AIR});
			CoreListener.fallingBlocks.put(fb.getUniqueId(), this);
			regenPreState.put(fb.getUniqueId(), b);
		}
		b.setType(Material.AIR);
		blocksDestroyed++;
		if (damagedBlocks.containsKey(b))
			damagedBlocks.get(b)[1] = Material.AIR;
	...
	public void triggerRegen() {
		if (WorldObject.findWorldObject(loc.getWorld()).difficulty != DifficultyLevel.CUSTOM || (int) WorldObject.findWorldObject(loc.getWorld()).settings.get("regenDelay") < 0)
			return;
		int count = 0;
		for (Map.Entry<Block, Material[]> entry : damagedBlocks.entrySet())
			if (entry.getValue()[0] == Material.AIR)
				count++;
...
//										}
									...
									regenStates.remove(entry.getKey());
