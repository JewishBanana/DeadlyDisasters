...
public class Blizzard extends WeatherDisaster {
	...
	public void start(World world, Player p, boolean broadcastAllowed) {
		...
		final double spawnRate = plugin.getConfig().getDouble("blizzard.mob_spawn_rate");
		EntityHandler handler = plugin.handler;
		String yetisBlessing = Languages.langFile.getString("misc.yetiBlessing");
								...
								if (all instanceof Player)
									...
									for (int i = 0; i < all.getHeight(); i++) {
										if (!Utils.passStrengthTest(eb.getType())) {
											...
											addBlockToList(eb, new Material[] { eb.getType(), Material.ICE });
				...
				RepeatingTask task = new RepeatingTask(plugin, (int) WorldObject.findWorldObject(world).settings.get("regenDelay") * 20, 1) {
					...
					public void run() {
						...
						while (regenTicks[0] >= 1) {
							...
							if (b.getType() == entry.getValue()[1] || ((b.getType() == Material.WATER || b.getType() == Material.LAVA) && ((Levelled) b.getBlockData()).getLevel() > 0)) {
								if (b.getType() == Material.ICE) {
									BlockBreakEvent event = new BlockBreakEvent(b, null);
									Bukkit.getPluginManager().callEvent(event);
									b.getWorld().spawnParticle(Particle.BLOCK_CRACK, b.getLocation().clone().add(.5,.5,.5), 10, .3, .3, .3, 0.01, Material.ICE.createBlockData());
