		...
		new RepeatingTask(plugin, 0, 1) {
			...
			public void run() {
				...
				if (tick >= radius) {
					...
					placements.forEach(i -> i.addTopBlock());
	...
	public void dig(Iterator<Places> it) {
		...
		for (int i=r; i > 0; i--) {
			...
			if (!Utils.passStrengthTest(b.getType()) && !Utils.isZoneProtected(b.getLocation())) {
				...
				if (!Utils.passStrengthTest(b2.getType()) && !Utils.isZoneProtected(b2.getLocation())) {
					if (depth == 1)
						classInstance.addBlockToList(b, new Material[] {b.getType(), b2.getType()});
					else
						classInstance.addBlockToList(b, new Material[] {b.getType(), Material.AIR});
		...
		if (depth <= 0) {
			if (loc.getBlockY() < (classInstance.maxD + 15)) {
				if (classInstance.damagedBlocks.containsKey(b))
					classInstance.damagedBlocks.replace(b, new Material[] {classInstance.damagedBlocks.get(b)[0], Material.LAVA});
				else
					classInstance.addBlockToList(b, new Material[] {b.getType(), Material.LAVA});
