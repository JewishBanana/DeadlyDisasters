...
public class SolarStorm extends WeatherDisaster {
	...
	public SolarStorm(int level) {
		super(level);
		time = plugin.getConfig().getInt("solarstorm.time.level "+this.level) * 20;
		delay = plugin.getConfig().getInt("solarstorm.start_delay") * 20;
		volume = plugin.getConfig().getDouble("solarstorm.volume");
		changeTime = plugin.getConfig().getBoolean("solarstorm.set_sunset");
		fireCatch = plugin.getConfig().getDouble("solarstorm.fire_catch_chance");
		fireSpawn = plugin.getConfig().getDouble("solarstorm.fire_spawn_rate");
		fireballSize = (float) plugin.getConfig().getDouble("solarstorm.fireball_explosion_size");
				...
				if (time > 0) {
					...
					for (LivingEntity e : world.getLivingEntities()) {
						...
						if (e instanceof Player && !Utils.isPlayerImmune((Player) e)) {
							...
							if (spawnMobs && ticksPerSpawn[0] >= 7 && e.getLocation().getBlockY() >= 60 && CustomEntityType.FIREPHANTOM.canSpawn() && rand.nextInt(3) == 0) {
								...
								plugin.handler.addEntity(new FirePhantom(entity, plugin, rand));
								entity.setTarget(e);
