...
public class Supernova extends DestructionDisaster {
	...
	public Supernova(int level) {
		...
		volume = plugin.getConfig().getDouble("supernova.volume");
		particleMultiplier = plugin.getConfig().getDouble("supernova.particle_multiplier");
		flash = plugin.getConfig().getBoolean("supernova.flash");
		this.sizeMultiplier = plugin.getConfig().getDouble("supernova.size");
		this.particle = Particle.EXPLOSION_LARGE;
		materials = new Material[]{Material.OBSIDIAN, Material.BLACK_CONCRETE, Material.FIRE};
		if (!plugin.getConfig().getBoolean("supernova.place_fire"))
			materials = new Material[]{Material.OBSIDIAN, Material.BLACK_CONCRETE};
		farParticles = plugin.getConfig().getBoolean("supernova.far_particles");
				...
				for (Block b : blocks) {
					...
					if (tick > size-2 && rand.nextInt(3) == 0) {
						if (rand.nextInt(3) == 0) {
							...
							addBlockToList(b, new Material[] {b.getType(), mat});
							b.setType(mat);
							blocksDestroyed++;
						}
						continue;
