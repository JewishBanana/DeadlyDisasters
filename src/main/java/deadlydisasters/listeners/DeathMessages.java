...
public class DeathMessages implements Listener {
	...
	public void onDeath(PlayerDeathEvent e) {
		...
		if (p.hasMetadata("dd-monsoondrown")) {
			...
			e.setDeathMessage(p.getName()+" "+Languages.langFile.getString("deaths.monsoon"));
		...
		if (p.hasMetadata("dd-devourdeath")) {
			...
			e.setDeathMessage(p.getName()+" "+Languages.langFile.getString("deaths.genericMob")+' '+Languages.langFile.getString("entities.infestedDevourer"));
			return;
		}
		if (p.hasMetadata("dd-infestedwormdeath")) {
			p.removeMetadata("dd-infestedwormdeath", plugin);
			e.setDeathMessage(p.getName()+" "+Languages.langFile.getString("deaths.infestedWormDeath"));
			return;
		}
			...
			if (world.getEnvironment() == Environment.NORMAL && acidstorms.size() != 0 && p.getLocation().getBlock().getTemperature() > 0.15 && p.getLocation().getBlock().getTemperature() <= 0.95) {
				if (p.getWorld().getHighestBlockAt(p.getLocation()).getY() <= p.getLocation().getBlockY()+1) {
					for (AcidStorm storm : acidstorms) {
						if (world.equals(storm.getWorld())) {
		...
		if (cause == DamageCause.SUFFOCATION) {
			for (InfestedCaves cave : infestedcaves)
				if (p.getWorld().equals(cave.getLocation().getWorld()) && p.getLocation().distance(cave.getLocation()) <= cave.getSize() && p.getLocation().add(0,1,0).getBlock().getType() == cave.getMaterial()) {
					e.setDeathMessage(p.getName()+" "+Languages.langFile.getString("deaths.infestedCave"));
					return;
				}
		}
