...
import deadlydisasters.disasters.SolarStorm;
...
public class Disasters implements CommandExecutor,TabCompleter {
	...
	private RepeatingTask regenTask;
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (args[0].equalsIgnoreCase("enable")) {
			...
			} else if (args[1].equalsIgnoreCase("yetifur")) {
				p.getInventory().addItem(ItemsHandler.yetifur);
				sender.sendMessage(Utils.chat("&aGave 1 [Yeti Fur] to "+p.getName()));
			} else if (args[1].equalsIgnoreCase("basicbook")) {
				p.getInventory().addItem(ItemsHandler.basicBook);
				sender.sendMessage(Utils.chat("&aGave 1 [Basic Book] to "+p.getName()));
			} else if (args[1].equalsIgnoreCase("poseidonstrident")) {
				p.getInventory().addItem(ItemsHandler.poseidonsTrident);
				sender.sendMessage(Utils.chat("&aGave 1 [Poseidon's Trident] to "+p.getName()));
			} else {
			...
			Map<Block,BlockData> blocksData = new HashMap<>();
			Iterator<Entry<RepeatingTask, Map<DisasterEvent, Map<Block, Material[]>>>> iterator = DisasterEvent.regeneratingTasks.entrySet().iterator();
			while (iterator.hasNext()) {
				Entry<RepeatingTask, Map<DisasterEvent, Map<Block, Material[]>>> entry = iterator.next();
				for (Entry<DisasterEvent, Map<Block, Material[]>> inMap : entry.getValue().entrySet()) {
					 blocks.putAll(inMap.getValue());
					 blocksData.putAll(inMap.getKey().blocksData);
				}
				entry.getKey().cancel();
				iterator.remove();
			}
			regenBlockSize = blocks.size();
			int blocksPerTick = plugin.getConfig().getInt("general.force_regen_blocks_per_tick");
			sender.sendMessage(Languages.prefix+Utils.chat("&bForce regenerating all damage... &eNote that this command creates intentional issues to speed up the process!"));
			int[] time = {0};
			regenTask = new RepeatingTask(plugin, 0, 1) {
				@Override
				public void run() {
					for (int i = 0; i < blocksPerTick; i++) {
						if (blocks.isEmpty()) {
							...
							sender.sendMessage(Languages.prefix+Utils.chat("&aRegenerated all "+regenBlockSize+" damaged blocks! \n&7&oTime: "
									+ String.format("%02d", (int)(Math.round(time[0] / 1000 / 60)))+":"+String.format("%02d", (int)(Math.round(time[0] / 1000 % 60)))+":"+String.format("%02d", (int)(Math.round(time[0] / 10 % 100)))));
							return;
						}
						Entry<Block, Material[]> entry = blocks.entrySet().iterator().next();
			...
			while (iterator.hasNext()) {
				Entry<RepeatingTask, Map<DisasterEvent, Map<Block, Material[]>>> entry = iterator.next();
				for (Entry<DisasterEvent, Map<Block, Material[]>> inMap : entry.getValue().entrySet())
					blocksSize += inMap.getValue().size();
				entry.getKey().cancel();
				iterator.remove();
			}
			if (regenTask != null) {
		...
		if (args.length == 1) {
			...
			if (args[0].equalsIgnoreCase("summon") && sender.hasPermission("deadlydisasters.summon")) {
				StringUtil.copyPartialMatches(args[1], Arrays.asList("babyendtotem","endtotem","endworm","voidguardian","voidarcher","voidstalker","lostsoul",
						"ancientskeleton","ancientmummy","tunneller","primedcreeper","skeletonknight","darkmage","soulreaper","yeti","normalMeteor","explodingMeteor","splittingMeteor","firephantom","curseddiver",
						"swampbeast","zombieknight","shadowleech","infestedskeleton","infestedzombie","infestedcreeper","infestedenderman","infestedspirit","infestedtribesman","infesteddevourer","infestedhowler",
						"infestedworm"), list);
				Collections.sort(list);
				return list;
			}
			if (args[0].equalsIgnoreCase("give") && sender.hasPermission("deadlydisasters.give")) {
				StringUtil.copyPartialMatches(args[1], Arrays.asList("plaguecure","voidshard","voidswrath","voidsedge","voidshield","ancientblade","ancientbone","ancientcloth","darkmagewand","soulripper","yetifur",
						"basicbook","poseidonstrident","splashplaguecure"), list);
				Collections.sort(list);
				return list;
			}
