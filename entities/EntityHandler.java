...
public class EntityHandler {
	...
	public void addEntityBySpecies(String species, Entity e) {
		...
		case "tamedlostsoul":
			...
			break;
		case "yeti":
			addEntity(new Yeti((Mob) e, plugin, rand));
			break;
		case "firephantom":
			addEntity(new FirePhantom((Mob) e, plugin, rand));
			break;
		case "curseddiver":
			addEntity(new CursedDiver((Mob) e, plugin, rand));
			break;
		case "swampbeast":
		...
		case "infestedskeleton":
			addEntity(new InfestedSkeleton((Mob) e, plugin));
			break;
		case "infestedzombie":
			addEntity(new InfestedZombie((Mob) e, plugin));
			break;
		case "infestedcreeper":
			addEntity(new InfestedCreeper((Creeper) e, plugin));
			break;
		case "infestedenderman":
			addEntity(new InfestedEnderman((Mob) e, plugin));
			break;
		case "infestedspirit":
			addEntity(new InfestedSpirit((Mob) e, plugin, rand));
			break;
		case "infestedtribesman":
			addEntity(new InfestedTribesman((Zombie) e, plugin, rand));
			break;
		case "infesteddevourer":
			addEntity(new InfestedDevourer((Zombie) e, plugin, rand));
			break;
		case "infestedhowler":
			addEntity(new InfestedHowler((Zombie) e, plugin, rand));
			break;
		case "elf":
...
//						if (e.getPersistentDataContainer().has(removalKey, PersistentDataType.BYTE)) {
//							
//						}
//			}
//		};
...
//		e.entity.addPotionEffect(new PotionEffect(PotionEffectType.GLOWING, 1000, 1, true, false));
		list.add(e);
		if (!running)
			startTimers();
		return e;
	...
	public Queue<CustomEntity> getList() {
		return list;
	}
}
