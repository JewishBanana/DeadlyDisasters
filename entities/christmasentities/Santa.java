...
import deadlydisasters.utils.Song;
...
public class Santa extends CustomEntity {
	...
	public Santa(Zombie entity, Main plugin, Random rand) {
		...
		bar = Bukkit.createBossBar(Utils.chat("&c"+Languages.getString("entities.santa")), BarColor.RED, BarStyle.SOLID, BarFlag.DARKEN_SKY, BarFlag.CREATE_FOG);
	}
	...
	public void tick() {
		if (entity == null)
			return;
		bar.setProgress((1.0 / entityType.getHealth()) * entity.getHealth());
	...
	public void function(Iterator<CustomEntity> it) {
		...
		if (entity.isDead()) {
			...
			CustomDropsFactory.generateDrops(entity.getLocation(), entityType, true);
			bar.removeAll();
			it.remove();
			clean();
			if (songMaker != null)
	...
	public static void summonSanta(Main plugin, Block block) {
		...
		SongMaker music = new SongMaker(Song.CHRISTMAS_THEME);
		for (Entity e : world.getNearbyEntities(block.getLocation(), 30, 30, 30))
			if (e instanceof Player)
				music.addPlayer((Player) e);
		music.setLoop(true);
		music.setVolume((float) ((double) CustomEntityType.SANTA.grabCustomSetting("boss_music_volume")));
		if ((boolean) CustomEntityType.SANTA.grabCustomSetting("play_boss_music"))
			music.playSong();
		new RepeatingTask(plugin, 0, 1) {
			@Override
			public void run() {
